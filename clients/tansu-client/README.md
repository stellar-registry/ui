...
contract `tansu-client` via Soroban RPC.
...
soroban contract bindings ts \
  ...
  --contract-id CBXKUSLQPVF35FYURR5C42BPYA5UOVDXX2ELKIM2CAJMCI6HXG2BHGZA \
  --output-dir ./path/to/tansu-client
```
...
```json
"dependencies": {
  "tansu-client": "./path/to/this/folder"
}
```
...
```json
"scripts": {
  "postinstall": "soroban contract bindings ts --rpc-url https://soroban-testnet.stellar.org --network-passphrase \"Test SDF Network ; September 2015\" --id CBXKUSLQPVF35FYURR5C42BPYA5UOVDXX2ELKIM2CAJMCI6HXG2BHGZA --name tansu-client"
}
```
...
```js
import { Contract, networks } from "tansu-client"
