# registry-manager-client JS

JS library for interacting with [Soroban](https://soroban.stellar.org/) smart
contract `registry-manager-client` via Soroban RPC.
...
soroban contract bindings ts \
  ...
  --contract-id CB4CNQJVA4PUQDHGTHNLAICM2A6TBBJBMU5YG6E3XTMWUISHJTC5BJ5Q \
  --output-dir ./path/to/registry-manager-client
```
...
```json
"dependencies": {
  "registry-manager-client": "./path/to/this/folder"
}
```
...
```json
"scripts": {
  "postinstall": "soroban contract bindings ts --rpc-url https://soroban-testnet.stellar.org --network-passphrase \"Test SDF Network ; September 2015\" --id CB4CNQJVA4PUQDHGTHNLAICM2A6TBBJBMU5YG6E3XTMWUISHJTC5BJ5Q --name registry-manager-client"
}
```
...
```js
import { Contract, networks } from "registry-manager-client"
